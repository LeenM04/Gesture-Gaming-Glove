...
namespace UnityStandardAssets.Characters.FirstPerson
{
    ...
    public class RigidbodyFirstPersonController : MonoBehaviour
    {
        ...
        {
    string path = Application.dataPath + "/serial_output.txt"; // Path to the IMU data file
    float gx = 0f, gz = 0f, gy = 0f;

    try
    {
        string[] values = File.ReadAllText(path).Trim().Split(',');
        if (values.Length >= 3)
        {   gx = float.Parse(values[0]);
            gy = float.Parse(values[1]);
            gz = float.Parse(values[2]);
    ...
    {
        // If file is not found or unreadable, return no input
        return Vector2.zero;
    }
//    Debug.Log($"IMU Data - gx: {gx}, gy: {gy}, gz: {gz}");

    Vector2 input = Vector2.zero;
    
    float gyroDeadZone = 3000f;

    if (Mathf.Abs(gx) > gyroDeadZone)
    {
        input.x = Mathf.Clamp(gx / 8000f, -1f, 1f);
    }

    if (Mathf.Abs(gz) > gyroDeadZone)
    {
        input.y = Mathf.Clamp(gz / 8000f, -1f, 1f);
    }

    if (Mathf.Abs(gy) > gyroDeadZone)
    {
        m_YRotation += -gy * 0.00005f;
    }

/*
    float gyroDeadZone = 600f; // You can tune this 
    if (Mathf.Abs(gx) > gyroDeadZone)
    {
    input.x = gx < 0 ? 1f : -1f;  // Right if gx > 0, left if gx < 0
    }
    if (Mathf.Abs(gz) > gyroDeadZone)
    {
        input.y = gz < 0 ? 1f : -1f;  // Forward if gx < 0, backward if gx > 0
    }
   */
    // Store rotation for RotateView() to handle
    m_YRotation += Mathf.Abs(gy) > gyroDeadZone ? -gy * 0.001f : 0f;
      ...
      private void RotateView()
{
    ...
    // Apply IMU-based yaw rotation directly
    transform.rotation = Quaternion.Euler(0f, m_YRotation, 0f);
    cam.transform.localRotation = Quaternion.identity;

    // Optionally align Rigidbody velocity with camera facing direction
